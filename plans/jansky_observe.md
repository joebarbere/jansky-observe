...
- **S1 (front-end A/B, `plans/77`, updated 2026-07-11)** — mostly bench, but its **arm D** (on-sky ground/cold-sky Y-factor Tsys of the Discovery feed during commissioning) runs through this app: two pointed captures with full SDR-settings provenance — which **M1 already delivers**. An ObservationType template ("Tsys sky/ground pair": point at ground → capture, point at cold sky → capture, same settings) is a natural M2 seed; the `sky_ground_tsys` reduction lives in jansky-research, not here.
- **S5, S6, S7** — no requirements on this app (S5/S6 are the meteor station's Echoes stack; S7's capture path is one SDR and arrives post-v1).
