...
- **ObservationType** — name ("Sun pointing calibration", "HI drift scan", "HI pointed", "RFI survey", "injection test", "Tsys sky/ground pair"), description, default SDR settings, and its **ChecklistTemplateItem** rows (ordered text + required flag). Different observation types → different procedure templates. Seeded types and their checklists: §5.4.
...
### 5.4 Seeded ObservationTypes — the observing ladder (added 2026-07-12)

The build's **first observing ladder** (four attended-session wins, [[hline_qa_hydrogen_rfi]]) ships as seeded ObservationTypes so the wizard walks a new station through them in order:

1. **Sun pointing calibration** — ladder win #1 and the prerequisite for everything pointed. Procedure source: the build's Stage 2.5 ([[hline_testing_and_first_light]]). Seeded ChecklistTemplateItems:
   - Mast plumbed (torpedo level) and angle gauge zeroed — *required*
   - Mast azimuth tape scale referenced to true north — *required*
   - Chain powered in Stage-1 order; injector current ~120 mA confirmed — *required*
   - Total-power readout running (live view or meter script) — *required*
   - Read Sun az/el prediction for *now* (re-read each round — it moves ~15°/hr) — *required*
   - Dial prediction onto scale + gauge; confirm power above cold-sky floor — *required*
   - Peak azimuth (±10° sweep, ~2° steps), then elevation; repeat both — *required*
   - Feed-shadow cross-check: shadow centered on dish at RF peak — *recommended*
   - Record Δaz/Δel offsets (scale/gauge vs predicted) into observation notes — *required* (the pointing model; the server stores them on the Station record and applies them to every future pointing display)
   - Expected ~1–3 dB rise at peak — log the actual number — *recommended*
2. **HI pointed — Cygnus region** (win #2): the first-light session; v1 classifier + HI4PI cross-check confirm it (§6).
3. **HI pointed — rotation curve longitudes** (win #3): repeat pointings; analysis lives in jansky-research/ezRA.
4. **Continuum drift scan — Cas A / Cyg A** (win #4, stretch): hours-long total-power drift; the rise-plateau-fall is the detection.

Plus the utility types: **Tsys sky/ground pair** (§10.1, feeds jansky-research plan 78), **RFI survey** (HackRF sweep), **injection test** (HackRF + pad stack). Seeding lands with M2 (data model + checklist templates); the Sun-cal type is the one whose offsets the server *consumes* (pointing display), so its Δaz/Δel fields are part of the M2 schema, not just note text.
...
- **M2 — observation records → `v0.3.0` (~2 weekends).** Data model + CRUD, checklist templates & persisted state, session wizard (astropy az/el/drift + NWS snapshot), notes, observers, seeded source list **+ seeded observing-ladder ObservationTypes incl. Sun pointing calibration with its Δaz/Δel pointing-model fields (§5.4)**. First release where the SQLite schema matters → forward-migration-on-start ships here. Claude: MCP surface mounts on the server, `/plan-session`, `/troubleshoot-chain`, `/new-migration` (§12.6).
...
- **S2 (H-line pipeline vs LAB/HI4PI, `plans/78`)** — per-pointing **calibrated** spectra with full provenance. The v2 cross-check (§6) is the same harness; share it (see §6). Covered by M3 *except* the calibration requirement (below). Also carries (since 2026-07-11, from S1's restructure) the **commissioning Tsys measurement** of the Discovery feed: a ground/cold-sky pointed-capture pair with full SDR-settings provenance — **M1 already delivers this**; a "Tsys sky/ground pair" ObservationType template is a natural M2 seed; the `sky_ground_tsys` reduction lives in jansky-research.
...
- **S1, S5, S6, S7** — no requirements on this app (S1's discrete A/B was deferred to the jansky-research backlog 2026-07-11 and its on-sky Tsys leg moved into S2 above; S5/S6 are the meteor station's Echoes stack; S7's capture path is one SDR and arrives post-v1).
