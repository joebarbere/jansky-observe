...
def summarize_sweep(csv_path: str | Path, top_n: int = 5) -> dict[str, Any]:
    ...
    ``top_n`` loudest are returned, loudest first. Malformed rows are skipped
    (the raw file stays authoritative); a CSV with no usable rows raises
    :class:`ValueError`.
    ...
    ranked = sorted(
        ((freq, total / count) for freq, (total, count) in sums.items()),
